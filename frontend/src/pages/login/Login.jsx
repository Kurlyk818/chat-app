...
const Login = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-w-96 mx-auto">
      <div className="w-full p-6 rounded-lg shadow-md bg-gray-400 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-0 gap">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Link to="/signup" className="text-sm hover:underline hover:text-blue-700 text-white mt-2 inline-block">
            {"Don't"} have an account?{" "}
          </Link>

          <div>
            <button className="btn btn-block btn-sm mt-2 border text-white border-slate-700" disabled={loading} > {loading ? (<span className="loading loading-spinner"></span>) : "Login" }  </button>
          </div>
        </form>
      </div>
    </div>
